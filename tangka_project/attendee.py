...
def put_delete_get_attendee(id):
    ...
    # must PUT method will remove the attendee from activities registered
            ...
            if FIRST_NAME in content.keys() and LAST_NAME in content.keys() and EMAIL in content.keys() \
                ...
                if curr_attendees:
                    ...
                    if len(curr_attendees) > 0:
                        ...
                        not_unique = False
                        for item_att in curr_attendees:
                            if item_att.key.id != id:
                                not_unique = True
                        if not_unique:
                            return send_json_msg(ATTENDEE_NOT_UNIQUE, 403, APPLICATION_JSON)
                ...
                # there's items that need to be removed
                    ...
                    attendee_item.pop(ACTIVITY_LIST, None)
                    attendee_item.update({
                        FIRST_NAME: content[FIRST_NAME],
                        LAST_NAME: content[LAST_NAME],
                        EMAIL: content[EMAIL],
        ...
        content = request.get_json()
        if content:
            # go though the keys to see if any is invalid
            for key_name in content.keys():
                if FIRST_NAME != key_name and LAST_NAME !=key_name and EMAIL !=key_name  \
                        and PHONE != key_name:
                    return send_json_msg(ATTENDEE_EXTRA_ATTR, 400, APPLICATION_JSON)
            ...
            if PHONE in content.keys():
                attendee_phone = content[PHONE]
            else:
                attendee_phone = attendee_item[PHONE]
            attendee_f_name =""
            if FIRST_NAME in content.keys():
                attendee_f_name = content[FIRST_NAME]
            else:
                attendee_f_name = attendee_item[FIRST_NAME]
            attendee_l_name = ""
            if LAST_NAME in content.keys():
                attendee_l_name = content[LAST_NAME]
            else:
                attendee_l_name = attendee_item[LAST_NAME]

            attendee_email = ""
            if EMAIL in content.keys():
                attendee_email = content[EMAIL]
            else:
                attendee_email = attendee_item[EMAIL]


            query = client.query(kind=ATTENDEES)
            query.add_filter(PHONE, "=", attendee_phone)
            query.add_filter(FIRST_NAME, "=", attendee_f_name)
            query.add_filter(LAST_NAME, "=", attendee_l_name)
            query.add_filter(EMAIL, "=", attendee_email)
            curr_attendees = list(query.fetch())
            if curr_attendees:
                # check if it is the same user or a different user
                if len(curr_attendees) > 0:
